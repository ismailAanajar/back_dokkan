...
@section('content')
<div x-data="updateCategory()" class="relative p-1">
    
    <form @submit.prevent="saveCategory" class="flex flex-wrap w-11/12 mt-14 mx-auto gap-5">
        <div class="flex-grow w-full sm:w-5/12">
            <label for="area_name" class="mb-2 text-sm font-medium text-gray-500">Category name</label>
            <input type="text" name="name" value="{{ $category->name }}" id="area_name" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 placeholder-slate-300" placeholder="area name">
        </div>    
        <div class="flex-grow w-full sm:w-5/12">
            ...
            return {
                ...
                saveCategory(e) {
                    ...
                    formData.append('id', '{{ $category->id }}');
                     axios.post('/admin/product/category/update', formData)
                        .then(res => {
                            console.log(res);
                            this.loading = false;
                            Toastify({
