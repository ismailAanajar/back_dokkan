...
@section('content')
<div x-data="createCategory()" class="relative p-1">
    
    <form @submit.prevent="updateCategory" class="flex flex-wrap w-11/12 mt-14 mx-auto gap-5">
        <div class="flex-grow w-full sm:w-5/12">
            <label for="area_name" class="mb-2 text-sm font-medium text-gray-500">Category name</label>
            <input type="text" name="name" id="area_name" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 placeholder-slate-300" placeholder="area name">
        </div>    
        <div class="flex-grow w-full sm:w-5/12">
            <label for="area_style" class="mb-2 text-sm font-medium text-gray-500">description</label>
            <textarea name="style" id="area_style" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 placeholder-slate-300" placeholder='{"color": "#f00"}'></textarea>    
        </div>
        <div class="w-full sm:w-5/12">
            <label for="image" class="block mb-2 text-stone-500 text-sm">Image</label>
            <img class="w-full h-auto max-h-56 p-1" :class="{' ring-2 ring-gray-300' : image}" :src="image" alt="">
        ...
        <div class="w-full">
            ...
            <a href="{{ route('admin.product.category.index') }}" type="submit" class="inline-block text-white bg-slate-900 hover:bg-slate-800 focus:outline-none focus:ring-4 focus:ring-green-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2 ">
                Back  
            </a>  
        </div>
    </form>
     ...
     const createCategory = () => {
            return {
                ...
                updateCategory(e) {
                    e.preventDefault();
                    this.loading = true;
                    axios.post('/admin/product/category/store', new FormData(document.querySelector('form')))
                        .then(res => {
                            console.log(res);
                            this.loading = false;
                            Toastify({
